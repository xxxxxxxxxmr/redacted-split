document
  ...
  .addEventListener("submit", async (event) => {
    ...
    const sourceWallet = JSON.parse(
      document.getElementById("source-wallet").value,
    );
    const tokenMint = document.getElementById("token-mint").value;

    if (!walletsFile || !sourceWallet || !tokenMint) {
      resultDiv.innerHTML =
    ...
    const sourceKeypair = solanaWeb3.Keypair.fromSecretKey(
      new Uint8Array(sourceWallet),
    );
    ...
    try {
      ...
      const tokensPerWallet = Math.floor(
        totalTokens.value.amount / numOfTargets,
      );
